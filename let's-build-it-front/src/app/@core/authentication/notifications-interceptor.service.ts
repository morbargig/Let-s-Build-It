  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    if (req?.params.get('autoNotification') === 'true') {
      return next.handle(req).pipe(
        tap((res) => {
          if (res.type !== 0) {
            // TODO fix res return {type = 0}
            let message = req?.params?.get(200?.toString());
            let entity = req?.params?.get('entity');
            _notificationService.handleError(res as HttpErrorResponse, entity, { [200]: message });
          }
          return res;
        }),
        catchError((err: HttpEvent<any>) => {
          if (err instanceof HttpErrorResponse) {
            ...
            let entity = req?.params?.get('entity');
            _notificationService.handleError(err as HttpErrorResponse, entity, {
              [err.status]: message,
            } as ErrorMessages);
          }
          return of(err);
