...
export class PaymentPlansAssignResolverService implements Resolve<ModalAssignPageModelConfig>  {
    ...
    resolve(): ModalAssignPageModelConfig | Observable<ModalAssignPageModelConfig> | Promise<ModalAssignPageModelConfig> {
        ...
        return {
            ...
            getAll: (evt) => this.partnersService.getAll(evt).pipe(map(res => {
                let pagedRes = {
                    data: res?.data?.map(i => {
                        return {
                            id: i.id,
                            img: i.logoImgId,
                            title: i.name,
                            detailsArr: [{ text: i?.status ? 'Active' : 'Inactive' }, { text: i.phone, icon: faPhone }, { text: i.vehiclesCount | 0, icon: faCar }]
                        } as PickModel
                    })
                    , total: res?.total
