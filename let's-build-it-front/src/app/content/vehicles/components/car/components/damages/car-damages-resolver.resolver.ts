    ...
    return {
      ...
      submit: (value: CarDamageModel) => {
        if (!!value?.id) {
          ...
          this.carDamagesService.update(this.sidebarService.entity.id, carDamage.id, value, errorMessages, entityName).toPromise().then(res => {
            if (!!res) {
              this.location.back();
            }
          })
          ...
          if (!!value.damageMediaItems?.length) {
            value.damageMediaItems = Array.from(value.damageMediaItems)
              .map(x => { return { mediaId: x.id } })
          }
          this.carDamagesService.create(this.sidebarService.entity.id, value).toPromise().then(res => {
            if (!!res) {
      ...
      getEntityById: (routeParams) => {
        if ('id' in routeParams) {
          this.carDamagesService.get(this.sidebarService.entity.id, routeParams.id).toPromise().then(res => {
            if (!!res) {
