...
export class AncillariesComponent extends TablePageDirective<AncillaryModel> implements OnInit {
    ...
    actions: [
      ...
      },
      {
        ...
        click: (evt: any) => {
          ...
          this.deleteItem(
            { type: 'Group', data: item },
            `Are you sure you want to delete '${item.ancillaryGroupName}' Ancillary Group? it will delete all is child Ancillaries`
          );
        },
      },
    ],
  ...
  public getDataProvider(evt: LazyLoadEvent, isExport?: boolean): Observable<IPagedList<AncillaryModel>> {
    if (evt?.sorts?.[0] !== 'AncillaryGroup.Name') {
  ...
  public deleteEntity(item: AncillaryModel | { type: string; data: AncillaryModel }): Observable<any> {
    ...
    if ('type' in item && item['type'] === 'Group') {
      ...
      this.ancillariesGroupService
        .delete(this.partnerId, item.data.ancillaryGroupId, errorMessages, entityName)
        .toPromise()
        .then((res) => {
          this.tc.getData();
        });
      ...
      this.ancillariesService
        .delete(this.partnerId, (item as AncillaryModel).id, errorMessages, entityName)
        .toPromise()
        .then((res) => {
          this.tc.getData();
        });
