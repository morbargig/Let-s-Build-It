...
export class AncillariesResolverService implements Resolve<ModalPageModelConfig>  {
  ...
  constructor(private entityService: AncillariesService, private entityFormService: AncillariesFormService, private sideBarPageService: SideBarPageService) { }
          ...
          for (let i in val) {
            if (i === 'documents')
              if (val[i] instanceof FileList) {
                ...
                if (val[i]?.length) {
                  val[i] = (val[i] as any)?.map((ii: any) => { return { id: ii?.id } })
                }
