...
export class PaymentMethodsComponent implements OnInit, OnDestroy {
  ...
  getData(): void {
    let evt = {
      page: 1,
      pageSize: 1,
      filters: { IsDefault: { matchMode: MatchMode.Equals, value: true } } as FilterObject,
    } as LazyLoadEvent;
    this.partnerBankAccountsService
      .getAll(this.partner.id, evt)
      .pipe(takeUntil(this.endded))
      .subscribe((res) => {
        if (res?.total) {
