  ...
  resolve(
    ...
    return {
      ...
      submit: (selected: PickModel, model: PartnerB2CSubscriptionModel) => {
        this.b2cSubscriptionsService
          .deleteAndMove(this.sidebarService.entity.id, model.id, selected.id)
          .toPromise()
          .then((res) => {
            if (!!res) {
      ...
      getAll: (evt) => {
        return this.b2cSubscriptionsService.getAll(this.sidebarService.entity.id, evt).pipe(
          map((res) => {
            let pagedRes = {
              data: res?.data?.map((i) => {
                return {
                  ...
                  title: i.name,
                  detailsArr: [
                    { text: i.customersCount, icon: faUser },
                    { text: i.isActive, icon: faStar },
                  ],
                } as PickModel;
              }),
              total: res?.total,
