...
export class B2BSubScriptionResolverService implements Resolve<ModalPageModelConfig> {
  constructor(
    private b2bSubscriptionsService: PartnerB2BSubscriptionService,
    ...
    return {
      ...
      submit: (value: { subscriptions: PartnerB2BSubscriptionModel[] }) => {
        if (!!value?.subscriptions?.length) {
          this.b2bSubscriptionsService
            .bulk(this.sidebarService.entity.id, value.subscriptions)
            .toPromise()
            .then((res) => {
              if (!!res) {
      ...
      getEntityById: (routePrams) => {
        this.b2bSubscriptionsService
          .getAll(this.sidebarService.entity.id, { page: 1, pageSize: MAX_INT })
          .toPromise()
          .then((res) => {
            b2bSubscriptionformControls = this.b2bSubscriptionFormService.generate(
              true,
              this.sidebarService.entity.id,
              res?.data
            );
