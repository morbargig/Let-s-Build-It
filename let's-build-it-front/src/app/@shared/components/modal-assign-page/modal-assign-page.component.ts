...
export class ModalAssignPageComponent implements OnInit {
  ...
  public set form(v: FormGroup) {
    ...
    this._form = v;
    if (!!this._form) {
      this.formChangedSubscription = this._form.valueChanges.pipe(debounceTime(300), takeUntil(this.ended)).subscribe(res => {
        ...
        for (const key in res) {
          if (Object.prototype.hasOwnProperty.call(res, key)) {
            ...
            if (!!controlConfiguration) {
              filters[key] = {
                matchMode: typeof filterValue === 'number' || controlConfiguration.config?.type === 'number' ? MatchMode.Equals : MatchMode.Contains,
                value: filterValue
              }
            }
        ...
        evt.page = 1;
        evt.pageSize = 10;
        this.modalPageConfig?.getAll(evt).toPromise().then((res) => {
          this.items = res?.data || []
        });
  ...
  constructor(
    ...
    this.route.data.pipe(takeUntil(this.ended)).subscribe((data) => {
      ...
      if (this.modalPageConfig.evt) {
        ...
        this.modalPageConfig?.getAll(this.modalPageConfig.evt as LazyLoadEvent).toPromise().then((res) => {
          this.items = res?.data || []
        });
      }
      else {
  ...
  public evtListener(evt: Subject<LazyLoadEvent>): void {
    ...
    this.evt.pipe(takeUntil(this.ended)).subscribe(res => {
      this.modalPageConfig?.getAll(res).pipe(takeUntil(this.ended)).subscribe((res) => {
        this.items = res?.data || []
      });
    })
  }
